	...
	<div class="container-login100" >
		<div class="wrap-login100 p-l-55 p-r-55 p-t-80 p-b-30">
			<form class="login100-form validate-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                ...
                Sew Fabric Update Product: <?php echo $_SESSION["jcode"];?>
                </span>
                ...
                <div class="login100-form-title p-b-37" data-validate="Select Personnel">
                    <select name="p_id" class="login100-form-btn">
                        <?php while($p_row = mysqli_fetch_array($all_personnel_result)):;?>
                        <option value="<?php echo $p_row['id']; ?>"><?php echo             $p_row['name']." ".$p_row["surname"] ?></option>
                        <?php endwhile; ?>
                    </select>   
                </div>
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if (isset($_POST['back'])) {
            ...
            header('Location:productOperationUI.php');  
                ...
                //unnecessary control sql line
                $query1 = "SELECT * FROM mercury.personnel_table WHERE id='".$_POST['p_id']."';";
                $p_response = mysqli_query($dbc, $query1);
                $query2 .= "SELECT * FROM mercury.machine_table WHERE id='".$_POST['m_id']."';";
                $m_response = mysqli_query($dbc, $query2);

            
                if($p_response and $m_response){
                    $query = "CALL mercury.UpdateSewFabric('".$_SESSION['product_id']."','".$_POST['p_id']."','".$_POST['m_id']."') ;";
