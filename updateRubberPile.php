<html lang="en">
<head>
	<title>Update Rubber Pile</title>
	<meta charset="UTF-8">
...
<?php
    ...
    $query ="SELECT * FROM mercury.personnel_table ;";
    $all_personnel_result3 = @mysqli_query($dbc, $query);
    $query ="SELECT * FROM mercury.personnel_table ;";
    $all_personnel_result4 = @mysqli_query($dbc, $query);
	...
	<div class="container-login100" >
		<div class="wrap-login100 p-l-55 p-r-55 p-t-80 p-b-30">
			<form class="login100-form validate-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
                ...
                Rubber Pile Update Product: <?php echo $_SESSION["jcode"];?>
                </span>
                ...
                <div class="login100-form-title p-b-37">
                    <select name="p3_id" class="login100-form-btn" data-validate="Select Personnel">
                        <?php while($p3_row = mysqli_fetch_array($all_personnel_result3)):;  ?>
                        <option value="<?php echo $p3_row['id']; ?>"><?php echo $p3_row['name']." ".$p3_row["surname"] ?></option>
                        <?php endwhile; ?>
                    </select>  
                </div>
                ...
                <div class="login100-form-title p-b-37">
                    <select name="p4_id" class="login100-form-btn" data-validate="Select Personnel">
                        <?php while($p4_row = mysqli_fetch_array($all_personnel_result4)):;  ?>
                        <option value="<?php echo $p4_row['id']; ?>"><?php echo $p4_row['name']." ".$p4_row["surname"] ?></option>
                        <?php endwhile; ?>
                    </select>  
                </div>
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        if (isset($_POST['back'])) {
            ...
            header('Location:productOperationUI.php');  
                ...
                $query3 = "SELECT * FROM mercury.machine_table WHERE id='".$_POST['p3_id']."';";
                $p3_response = @mysqli_query($dbc, $query3);
                $query4 = "SELECT * FROM mercury.machine_table WHERE id='".$_POST['p4_id']."';";
                $p4_response = @mysqli_query($dbc, $query4);
